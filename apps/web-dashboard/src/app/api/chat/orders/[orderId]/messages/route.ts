...
import { sendPushToUser, sendPushToRestaurant } from "@/lib/firebase/send-push";
...
export async function POST(
    ...
    try {
        ...
        // Send FCM push notification to the recipient (fire-and-forget)
        const pushDb = db as any;
        const preview = content.trim().length > 60
            ? content.trim().slice(0, 57) + "…"
            : content.trim();

        if (isCustomer) {
            // Customer sent → notify restaurant members
            sendPushToRestaurant(pushDb, order.restaurant_id, {
                title: "💬 Nouveau message client",
                body: preview,
                data: { conversationId: conv.id, orderId, type: "chat_message" },
                link: `/dashboard/orders/${orderId}`,
            }).catch((e) => console.error("[Chat] push to restaurant failed:", e));
        } else {
            // Merchant sent → notify customer (if order has a customer)
            if (order.customer_id) {
                sendPushToUser(pushDb, order.customer_id, {
                    title: "💬 Réponse du restaurant",
                    body: preview,
                    data: { conversationId: conv.id, orderId, type: "chat_message" },
                    link: `/stores/orders/${orderId}`,
                }).catch((e) => console.error("[Chat] push to customer failed:", e));
            }
        }
