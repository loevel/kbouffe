// @ts-nocheck
/**
 * PATCH /api/restaurant/product-reviews/[id]
 * Permet au commerçant de répondre à un avis produit.
...
export async function PATCH(request: NextRequest, { params }: { params: Params }) {
    ...
    try {
        ...
        // Vérifier que l'avis produit appartient bien à ce restaurant
        // Vérifier que l'avis produit appartient bien à ce restaurant
        ...
        if (error) {
            console.error("[PATCH /api/restaurant/product-reviews/:id]", error);
            return apiError("Erreur lors de la mise à jour");
        }

        return NextResponse.json({ success: true });
    } catch (err) {
        console.error("[PATCH /api/restaurant/product-reviews/:id] unexpected", err);
