...
export function ClientStoresChrome({
    ...
    return (
        <div className="min-h-screen bg-white dark:bg-surface-950 flex transition-colors duration-500">
            {/* ── Sidebar ────────────────────────────────────────────────── */}
            <aside className="hidden lg:flex lg:w-72 lg:flex-col border-r border-surface-100 dark:border-white/5 bg-surface-50/50 dark:bg-surface-900/30 backdrop-blur-xl">
            ...
            <div className="flex-1 min-w-0 flex flex-col items-center">
                <header 
                    className={`fixed top-0 right-0 z-40 transition-all duration-500 h-20 flex items-center gap-4 px-6 lg:px-10 ${
                        scrolled 
                ...
                >
                    <button
                        className="w-12 h-12 rounded-2xl bg-white dark:bg-surface-900 border border-surface-100 dark:border-white/5 flex items-center justify-center text-surface-700 dark:text-surface-300 lg:hidden shadow-xl"
                        onClick={() => setIsAccountDrawerOpen(true)}
                    >
                        <Menu size={24} />
                    </button>

                    <nav className="hidden md:flex items-center gap-2 p-1.5 bg-surface-100 dark:bg-surface-900 rounded-2xl border border-surface-200 dark:border-white/5 shadow-inner">
                        {["delivery", "pickup", "reservation"].map((mode) => (
                            <button 
                                key={mode}
                                onClick={() => updateFilters({ deliveryMode: mode as any })}
                                className={`px-6 py-2.5 rounded-[0.9rem] text-[10px] font-black uppercase tracking-widest transition-all ${
                    ...
                    <div className="relative">
                        <button 
                            ...
                            className="hidden lg:flex items-center gap-3 px-5 py-3 rounded-2xl text-xs font-black uppercase tracking-widest text-surface-700 dark:text-surface-200 bg-white dark:bg-surface-900 border border-surface-100 dark:border-white/5 shadow-lg hover:shadow-2xl transition-all"
                        >
                        ...
                        <AnimatePresence>
                            {isCityMenuOpen && (
                                <>
                                    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-40" onClick={() => setIsCityMenuOpen(false)} />
                                    <motion.div 
                                        initial={{ opacity: 0, y: 15, scale: 0.95 }} 
                                        animate={{ opacity: 1, y: 0, scale: 1 }} 
                                        exit={{ opacity: 0, y: 15, scale: 0.95 }}
                                        className="absolute top-full left-0 mt-3 w-64 bg-white dark:bg-surface-900 rounded-3xl shadow-[0_40px_80px_-16px_rgba(0,0,0,0.3)] border border-surface-100 dark:border-white/5 p-3 z-50 overflow-hidden backdrop-blur-xl"
                                    >
                                        ...
                                        <button onClick={handleAutoDetectLocation} disabled={isDetectingLocation} className="w-full flex items-center gap-3 px-4 py-4 mb-3 rounded-2xl text-xs font-black text-brand-600 dark:text-brand-400 bg-brand-50 dark:bg-brand-500/10 border-2 border-dashed border-brand-200 dark:border-brand-500/20 transition-all hover:scale-[1.02]">
                                            ...
                                            {isDetectingLocation ? "DÉTECTION..." : "UTLISER MA POSITION"}
                                        </button>
                    ...
                    <div className="flex items-center gap-4 ml-auto">
                        ...
                        <Link
                            href="/stores/cart"
                            className="w-14 h-14 rounded-2xl bg-white dark:bg-surface-900 text-surface-700 dark:text-surface-200 flex items-center justify-center relative shadow-xl hover:shadow-brand-500/20 border border-surface-100 dark:border-white/5 transition-all group"
                        >
                            <ShoppingCart size={22} className="group-hover:rotate-12 transition-transform" />
                            {itemCount > 0 && (
                                <span className="absolute -top-2 -right-2 bg-brand-500 text-white text-[10px] font-black rounded-lg w-6 h-6 flex items-center justify-center shadow-xl border-2 border-white dark:border-surface-900 animate-bounce">
                ...
                <main className="w-full max-w-7xl px-6 lg:px-10 pt-32 pb-20">
