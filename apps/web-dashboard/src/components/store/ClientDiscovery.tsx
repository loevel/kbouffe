...
// ── Constants ────────────────────────────────────────────────────────────────
function getPromoCards(t: TranslationKeys) {
...
function getFilterChips(t: TranslationKeys) {
    const d = t.clientDiscovery;
    return [
        { id: "offers", label: d.chipOffers, href: "/stores/offers" },
        { id: "free_delivery", label: d.chipFreeDelivery },
        { id: "fast", label: d.chipFast },
        { id: "premium", label: d.chipPremium },
    ];
}
    ...
    return (
        <div className="flex items-center gap-3 overflow-x-auto scrollbar-hide pb-4 -mx-4 sm:-mx-6 px-4 sm:px-6 mb-4">
            ...
            >
                ...
                <span className="text-[11px] font-medium whitespace-nowrap">{t.clientDiscovery.all}</span>
            </button>
            {categories.map((c) => (
                <button
                    key={c.value}
            ...
            <div className="relative shrink-0">
                ...
                {sortOpen && (
                    <div className="absolute top-full mt-1 left-0 w-44 bg-white dark:bg-surface-900 border border-surface-200 dark:border-surface-700 rounded-xl shadow-xl z-20 overflow-hidden">
                        {[
                            { id: "rating", label: d.sortRating },
                            { id: "recommended", label: d.sortRecommended },
                            { id: "orders", label: d.sortOrders },
                        ].map((opt) => (
                            <button
                                key={opt.id}
                                onClick={() => {
...
function PromoCarousel() {
    const { t } = useDashboardLocale();
    const promoCards = getPromoCards(t);
            ...
            <div className="relative h-36 rounded-2xl overflow-hidden mb-3 bg-surface-100 dark:bg-surface-800">
                ...
                {r.isSponsored && (
                    <div className="absolute top-2 left-2 flex items-center gap-1 bg-brand-600 text-white text-[10px] font-bold px-2 py-1 rounded-full max-w-[70%] truncate">
                        <Ticket size={10} className="shrink-0" /> {d.sponsoredBadge}
                    </div>
                )}
                ...
                <button
                    ...
                    aria-label={isFavorite ? d.removeFromFavorites : d.addToFavorites}
                >
...
function NearbyCircles({ items, title }: { items: RestaurantItem[]; title?: string }) {
    const { t } = useDashboardLocale();
    const d = t.clientDiscovery;
    ...
    return (
        <div className="mb-10">
            <div className="flex items-center justify-between mb-4">
                <h2 className="text-xl font-extrabold text-surface-900 dark:text-white">{title ?? d.nearby}</h2>
            ...
            <div ref={rowRef} className="flex gap-5 overflow-x-auto scrollbar-hide pb-2 -mx-4 sm:-mx-6 px-4 sm:px-6">
                {items.map((r) => (
                    <Link key={r.id} href={`/r/${r.slug}`} className="shrink-0 flex flex-col items-center gap-2 group w-20">
                        ...
                        <span className="text-[10px] text-surface-400">{d.schedule}</span>
                    </Link>
                ))}
            </div>
        </div>
...
export function ClientDiscovery() {
    ...
    }, [filters.cuisineTypes, filters.city, filters.coords, d.errorLoad]);
    ...
    return (
        <div className="min-h-full bg-white dark:bg-surface-950 overflow-x-hidden">
            ...
            <p className="text-xs text-surface-400 dark:text-surface-600 mb-8">
                {d.legalNotice}{" "}
                <Link href="/legal" className="underline hover:text-surface-600">{d.learnMore}</Link>
            </p>
            ...
            {loading ? (
                ...
                <div className="text-center py-24 bg-surface-50 dark:bg-surface-900/40 rounded-2xl border-2 border-dashed border-surface-200 dark:border-surface-800">
                    ...
                    <p className="text-surface-400 text-sm mb-5">{d.noResultsDesc}</p>
                    <button
                        onClick={() => {
