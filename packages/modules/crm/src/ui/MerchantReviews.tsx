...
import { Card, EmptyState, useLocale, formatDate } from "@kbouffe/module-core/ui";
import { useMerchantReviews, type MerchantReview } from "./useReviews";
...
export function MerchantReviewsPage() {
    ...
    const { reviews, total, totalPages, isLoading, reload } = useMerchantReviews(page);
    ...
    const handleRespond = async (reviewId: string, response: string) => {
        const res = await fetch(`/api/restaurant/reviews/${reviewId}`, {
            method: "PATCH",
        ...
        <div>
            ...
            {isLoading ? (
                ...
                <EmptyState
                    ...
                    title="Aucun avis"
                    description="Vos clients n'ont pas encore laissé d'avis."
                />
            ) : (
                <div className="space-y-3">
                    {filtered.map((review) => (
